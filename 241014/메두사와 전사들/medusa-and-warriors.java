...
//    	System.out.println(notStones);
    ...
    static class Warrior{
    	...
    	public String toString() {
    		return String.format("%d: (%d,%d)", id,y,x);
    	}
    	
    	public Move move(int visited[][]) {
    		
    		// 첫번째 이동 
    		int distance = getDistance(y,x, monster.y, monster.x);
    		int direction = -1;
    		for(int dir = 0; dir< 4; dir++) {
    			int ny = y + dy[dir];
    			int nx = x + dx[dir];
    		...
    		wBoard[y][x] -= value; // 이동전 
    		
    		y = y + dy[direction];
    		x = x + dx[direction];
    		
    		if(monster.isSame(y, x)) { // 첫번째 이동을 하고 메두사와 같은 칸에 도달했다
    			warriors[id] = DEAD;
    			return new Move(1, true);
    		}
    		wBoard[y][x] += value; // 이동후 
    		...
    		if(monster.isSame(y, x)) { // 두번째 이동을 하고 메두사와 같은칸에 도달했다 
    			warriors[id] = DEAD;
