...
public class Main {
	...
	static class Turret implements Comparable<Turret>{
		...
		public boolean isWeaker(Turret t) {
			if(power != t.power)
				return power < t.power;
			if(time != t.time)
				return t.time < time;
		...
		public boolean isStronger(Turret t) {
			if(power != t.power)
				return power > t.power;
			if(time != t.time)
				return t.time > time;
	...
	static Turret STORNGEST = new Turret(5001,-1,-1,-1);
	static Turret WEAKEST = new Turret(-1,1001,12,12);
...
//		target = turrets.get(turrets.size() - 1);
		...
		for(int y=1; y<=n; y++) {
			for(int x=1; x<=m; x++) {
				...
				if(board[y][x] == BROKEN)
					continue;
				if(board[y][x].isWeaker(attacker)) {
					attacker = board[y][x];
				}
				
				if(board[y][x].isStronger(target)) {
					target = board[y][x];
				}
			}
		}
